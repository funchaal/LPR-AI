...
def init_ocr(det_model_dir, rec_model_dir, use_det=True, device='cpu', char_dict_file=None, logger: logging.Logger = None, **kwargs):
    """
    ...
    """
    logger.info("--- Iniciando seleção de backend de OCR ---")

    # Se o dispositivo for GPU, a escolha é direta: ONNX com CUDA.
    if 'gpu' in device:
        logger.info("Dispositivo GPU detectado. Usando backend ONNX com CUDA.")
        try:
            ...
            logger.critical(f"Falha ao inicializar o backend ONNX para GPU: {e}", exc_info=True)
    ...
    # Se o dispositivo for CPU, tentamos a otimização com OpenVINO primeiro.
        ...
        try:
            logger.info("Dispositivo CPU detectado. Tentando backend otimizado OpenVINO...")
            ...
            logger.warning(f"Backend OpenVINO não pôde ser inicializado. Erro: {e}")
            logger.info("Fallback para o backend ONNX com CPUExecutionProvider.")
            ...
            try:
                ...
                logger.critical(f"Falha ao inicializar o backend de fallback ONNX para CPU: {e_onnx}", exc_info=True)
