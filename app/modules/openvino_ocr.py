...
class OCR:
    ...
    def __init__(self, det_model_dir, rec_model_dir, cls_model_dir=None, use_angle_cls=False, use_det=True, char_dict_path=None):
        ...
        if self.use_det:
            logging.info("Carregando modelo de Detecção (OpenVINO)...")
        ...
        logging.info("Carregando e configurando modelo de Reconhecimento (OpenVINO)...")
        ...
        logging.info(f"DEBUG: Caminho do dicionário recebido: {char_dict_path}")
        logging.info(f"DEBUG: Tamanho do dicionário de caracteres carregado: {len(self.character)}")
    ...
    def _unclip(self, box, unclip_ratio):
        ...
        try:
            ...
            logging.warning(f"Erro ao expandir caixa: {e}")
            return None
    ...
    def _get_mini_boxes(self, contour):
        ...
        try:
            ...
            logging.warning(f"Erro em _get_mini_boxes: {e}")
            return None, 0
    ...
    def _postprocess_detection(self, pred, shape_list):
        ...
        """
        # CORREÇÃO: Garante que pred tenha as dimensões corretas
        ...
        for batch_index in range(pred.shape[0]):
            ...
            # CORREÇÃO: Garante que mask seja 2D
            if len(mask.shape) > 2:
                mask = mask.squeeze()
    ...
    def _boxes_from_bitmap(self, pred, bitmap, dest_width, dest_height):
        ...
        for index in range(num_contours):
            ...
            try:
                ...
                logging.warning(f"Erro ao calcular score: {e}")
                continue
            ...
            try:
                ...
                logging.warning(f"Erro ao obter mini boxes após expansão: {e}")
                continue
...
def init_openvino_ocr(det_model_dir=None, rec_model_dir=None, use_det=True, char_dict_file=None, **kwargs):
    """
    ...
    def find_openvino_model_path(base_dir: str) -> str | None:
        ...
        if not os.path.isdir(openvino_subdir_path):
            logging.warning(f"Subdiretório OpenVINO esperado não foi encontrado em: {openvino_subdir_path}")
            return None
        ...
        for filename in os.listdir(openvino_subdir_path):
            if filename.endswith(".xml"):
                ...
                logging.info(f"Modelo OpenVINO encontrado: {model_path}")
                return model_path
        
        logging.warning(f"Nenhum arquivo .xml de modelo encontrado no diretório: {openvino_subdir_path}")
        return None
    ...
    logging.info("Motor de OCR OpenVINO inicializado com sucesso.")
    return ocr_engine